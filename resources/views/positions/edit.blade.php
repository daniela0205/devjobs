...
<h1 class="text-2xl text-center mt-10">Edit Positions : {{$position->title}}</h1>

<form
action="{{ route('positions.update', ['position' => $position->id ]) }}"
method="POST"
...
<div class="mb-5">
    ...
    >
      ...
      @foreach ($categories as $category)
            <option
                {{ $position->category_id == $category->id ? 'selected' : '' }}
                value="{{ $category->id }}"
            >
                {{$category->name}}
            </option>
...
<div class="mb-5">
    ...
    >
        ...
        @foreach ($experiences as $experience)
            <option
                {{ $position->experience_id == $experience->id ? 'selected' : '' }}
                value="{{ $experience->id }}"
            >
                {{$experience->name}}
            </option>
...
<div class="mb-5">
    ...
    >
        ...
        @foreach ($locations as $location)
            <option
                {{ $position->location_id == $location->id ? 'selected' : '' }}
                value="{{ $location->id }}"
            >
                {{$location->name}}
            </option>
...
<div class="mb-5">
    ...
    >
        ...
        @foreach ($salaries as $salary)
            <option
                {{ $position->salary_id == $salary->id ? 'selected' : '' }}
                value="{{ $salary->id }}"
            >
                {{$salary->name}}
            </option>
...
<div class="mb-5">
    ...
    <input type="hidden" name="imagen" id="imagen" value="{{ $position->imagen }}" >
    @error('imagen')
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mt-3 mb-6" role="alert">
            <strong class="font-bold">Error!</strong>
            <span class="block"> {{$message}}</span>
...
<div class="mb-5">
    ...
    <list-skills
        ...
        :oldskills="{{ json_encode($position->skills ) }}"
    ></list-skills>
    @error('skills')
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mt-3 mb-6" role="alert">
            <strong class="font-bold">Error!</strong>
            ...
            const dropzoneDevJobs = new Dropzone('#dropzoneDevJobs', {
                ...
                init: function() {
                    if(document.querySelector('#imagen').value.trim() ) {
                       ...
                       imagenPosted.nameServer = document.querySelector('#imagen').value
